...
public class CanNotAddMoraleHandle extends BaseHandle {

	private static Logger logger = Logger.getLogger(CanNotAddMoraleHandle.class);
	...
	public void handle(Hero hero, Buff buff, Context context) {
		logger.debug("处理不能增加士气buff.name[" + hero.getLogName() + "]");
		hero.setCanNotAddMorale(true);
	}

	public void handleRemove(Hero hero, Buff buff) {
		logger.debug("清除不能增加士气状态.hero.Name[" + hero.getLogName() + "], effect.Remark[" + buff.getEffect().getRemark() + "]");
		hero.setCanNotAddMorale(false);
	}
