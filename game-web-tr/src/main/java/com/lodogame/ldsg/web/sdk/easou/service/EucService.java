...
public class EucService {
	...
	private void init(Properties pro) {
		try {
			...
			apiServerIOS=ClientConfig.getProperty("apiServerIOS");
		} catch (Exception e) {
			// log.error(e, e);
			e.printStackTrace();
		}
	...
	@SuppressWarnings("unchecked")
	public JBean getResultIOS(String path, JBody jBody, AuthParametric authPara, RequestInfo info) throws EucAPIException {
		// TODO 生成公共请求头
		JHead jHead = authPara.getVeriHeader(jBody, this, info);
		// TODO 生成请求参数
		JBean jbean = new JBean();
		...
		System.out.println(requestJSON);
		// TODO 请求接口并返回结果
		String result = executeUrl(buildURL(apiServerIOS + path, info), requestJSON);
		System.out.println(result);
		if (null == result || "".equals(result)) {
			return null;
		}
