...
public class GooglePlayController {
	private static Logger LOGGER = Logger.getLogger(GooglePlayController.class);
	...
	@RequestMapping(value = "/webApi/googlePayment.do")
	public ModelAndView payCallback(HttpServletRequest req, HttpServletResponse res) {
		PartnerService ps = serviceFactory.getBean(GooGlePlaySdk.instance().getPartnerId());
		...
		LOGGER.info("google 支付请求数据 json[" + json + "] signature[" + signature + "]");
		
		try {
			if(StringUtils.isNotEmpty(json)	&& StringUtils.isNotEmpty(signature)){

				if (Security.verifyPurchase(GooGlePlaySdk.instance().getAppKey(), json, signature)) {
